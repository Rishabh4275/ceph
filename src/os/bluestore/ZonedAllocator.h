...
class ZonedAllocator : public Allocator {
  ...
  std::atomic<uint64_t> num_free;     ///< total bytes in freelist
  uint64_t size;
  uint64_t block_size;
  uint64_t zone_size;
  ...
  std::atomic<uint64_t> num_zones_to_clean;
...
public:
  ...
  uint64_t get_num_zones_to_clean(void) {
    return num_zones_to_clean;
  }
