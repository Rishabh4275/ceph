    ...
    else if (b->get_blob().is_compressed())
    {
      ...
      BlobInfo &bi =
          affected_blobs.emplace(
                            b, BlobInfo(b->get_referenced_bytes()))
              .first->second;
  ...
  while (i != buffer_map.end())
  {
    ...
    if (b->offset < offset)
    {
      ...
      if (b->end() > end)
      {
        ...
        {
          _add_buffer(cache, new Buffer(this, b->state, b->seq, end, tail, b->flags),
                      0, b);
        }
        if (!b->is_writing())
        {
    ...
    {
      _add_buffer(cache, new Buffer(this, b->state, b->seq, end, keep, b->flags),
                  0, b);
    }
    _rm_buffer(cache, i);
  ...
  while (true)
  {
    ...
    if (p->second->offset < pos)
    {
      ...
      if (p->second->data.length())
      {
        ...
        r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, 0, bl, p->second->flags),
                      0, p->second.get());
      }
      else
      {
        r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, 0, right, p->second->flags),
                      0, p->second.get());
      }
    ...
    if (p->second->data.length())
    {
      r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, p->second->offset - pos, p->second->data, p->second->flags),
                    0, p->second.get());
    }
    else
    {
      r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, p->second->offset - pos, p->second->length, p->second->flags),
                    0, p->second.get());
    }
    ...
    for (it->lower_bound(string()); it->valid(); it->next())
    {
      ...
      if (key == BLUESTORE_GLOBAL_STATFS_KEY)
      {
        if (repairer)
        {
          ...
          derr << "fsck error: "
               << "legacy statfs record found, removing"
               << dendl;
        }
        continue;
      }
    ...
    const BlueStore::FSCK_ObjectCtx &ctx)
{
  ...
  store_statfs_t *res_statfs = (per_pool_stat_collection || repairer) ? &ctx.expected_pool_statfs[pool_id] : &ctx.expected_store_statfs;
  ...
  for (auto &i : ref_map)
  {
    ...
    bool equal =
        depth == FSCK_SHALLOW ? true : i.first->get_blob_use_tracker().equal(i.second);
    if (!equal)
    {
...
{
public:
  ShallowFSCKThreadPool(CephContext *cct_, std::string nm, std::string tn, int n) : ThreadPool(cct_, nm, tn, n)
  {
  }
  ...
  struct FSCKWorkQueue : public ThreadPool::WorkQueue_
  {
    ...
    FSCKWorkQueue(std::string n,
                  ...
                  BlueStoreRepairer *_repairer) : WorkQueue_(n, ceph::timespan::zero(), ceph::timespan::zero()),
...
int BlueStore::_fsck(BlueStore::FSCKDepth depth, bool repair)
{
  dout(1) << __func__
          << (repair ? " repair" : " check")
          << (depth == FSCK_DEEP ? " (deep)" : depth == FSCK_SHALLOW ? " (shallow)"
                                                                     : " (regular)")
          << dendl;
...
void BlueStore::_get_statfs_overall(struct store_statfs_t *buf)
{
  ...
  auto prefix = per_pool_omap == OMAP_BULK ? PREFIX_OMAP : per_pool_omap == OMAP_PER_POOL ? PREFIX_PERPOOL_OMAP
                                                                                          : PREFIX_PERPG_OMAP;
  buf->omap_allocated =
  ...
  if (bluefs)
  {
    ...
    buf->internal_metadata =
        bluefs->get_used() - buf->omap_allocated;
  }
                           ...
                           bool *out_per_pool_omap)
{
  ...
  *out_per_pool_omap = per_pool_omap != OMAP_BULK;
  ...
  {
    auto prefix = per_pool_omap == OMAP_PER_POOL ? PREFIX_PERPOOL_OMAP : PREFIX_PERPG_OMAP;
  ...
  while (length > 0)
  {
    ...
    if (!wctx->compress)
    {
          ...
          l <= prefer_deferred_size_snapshot * 2)
      {
        ...
        bool will_defer = ep != end ? head_info.can_defer(ep,
        ...
        if (will_defer && remaining)
        {
          ...
          if (remaining <= prefer_deferred_size_snapshot)
          {
            ...
            will_defer = ep_next == end ? false : tail_info.can_defer(ep_next, prefer_deferred_size_snapshot, block_size, offset_next, remaining);
